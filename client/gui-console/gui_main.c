...
char *next_word(char const **s, char const *ifs)
{
  ...
  *s += strspn(*s, ifs);
  ...
  if (first_delimiter) {
    ...
    fc_strlcpy(word, *s, first_delimiter - *s + 1);
    *s = first_delimiter;
  } else {
    word = fc_strdup(*s);
    *s = NULL;
...
static void console_unit_summary(struct unit *punit, char const *prefix)
{
  fc_printf("%s- %d. (%d, %d) %s (%s) %d/%d [%s]\n", prefix,
	    ...
	    rule_name(&utype_veteran_level(punit->utype, punit->veteran)->name),
...
void console_command(char const *s)
{
  ...
  if (n_words) {
    for (i = 0; handlers[i].command; i++) {
      if (fc_strcasecmp(handlers[i].command, words[0]) == 0) {
...
void console_move(int argc, char *argv[], void *context)
{
  ...
  for (i = 0; directions[i].name; i++) {
    if (fc_strcasecmp(directions[i].name, argv[0]) == 0) {
...
void console_politics(int argc, char *argv[], void *context)
{
  ...
  players_iterate(pplayer) {
    players_iterate(other_player) {
      ...
      diplstate = player_diplstate_get(pplayer, other_player);
...
void console_fullmap(int argc, char *argv[], void *context)
{
  ...
  whole_map_iterate(ptile) {
    ...
    index_to_map_pos(&tile_x, &tile_y, tile_index(ptile));
    xy = columns_per_tile*(max_x+1)*(rows_per_tile*tile_y) + columns_per_tile*tile_x + columns_per_tile/2;
...
void console_statc(int argc, char *argv[], void *context)
{
  ...
  for (traderoute = 0; traderoute < NUM_TRADE_ROUTES; traderoute++) {
    if (pcity->trade[traderoute]) {
      ...
      trade_partner = game_city_by_number(pcity->trade[traderoute]);
...
void ui_main(int argc, char *argv[])
{
  ...
  if (stdin_flags == -1) {
    log_error("Couldn't get stdin file flags. It may still be in blocking mode.");
  } else if (!(stdin_flags & O_NONBLOCK)) {
#if 0
    if (fcntl(0, F_SETFL, stdin_flags | O_NONBLOCK) == -1) {
      log_error("Couldn't set stdin to non-blocking mode.");
    }
#endif
  }
